...
const ProductReq = () => {
  ...
  useEffect(() => {
    ...
    getProductReq();
  }, [accessToken,pageNumber,ProductReq]);

  const getProductReq = () => {
    axios({
      method: "GET",
      url: `${BaseUrl}/productrequest/v1/getRequestedProductsByPagination/{pageNumber}/{pageSize}/{technicianId}?pageNumber=${pageNumber}&pageSize=${pageSize}&technicianId=${techId}`,
      headers: {
      ...
      setProductReq(result.data.content);
    });
  };
...
const viewItem = ({item}) => {
  return (
     ...
     <Card style={styles.card}>
      <Card.Content >   
     <Text style={styles.header}>{item.productDto===null ? "No product name":item.productDto.productName}</Text>
     </Card.Content>
