...
import BaseUrl from "../api/BaseUrl";
  ...
  const CollectionSide = () => {
    ...
    const onTaskSide = (id) => {
      // console.warn("HI"+id)
      navigation.navigate("collectionClick");
      AsyncStorage.setItem ("collectiontaskId",JSON.stringify(id));
    };
    ...
    const getCollectionPage = () => {
      axios({
        method: "GET",
        url: `${BaseUrl}/collectiontask/v1/getAllCollectionTaskByPagination/{pageNumber}/{pageSize}/{technicianId}?pageNumber=${pageNumber}&pageSize=${pageSize}&technicianId=${techId}`,
        headers: {
        ...
        setCollectionPage(result.data.content);
      });
    };
  ...
  const viewItem = ({item}) => {
    ...
    return (
      <Card style={styles.ccard} onPress={()=>{
        onTaskSide(item.collectiontaskId)
      }}>
      <View style={{flexDirection:"row" ,backgroundColor:"skyblue"}}>
      <Text style={styles.ttitle}>{item.collectiontaskName}</Text>
   <TouchableOpacity onPress = {()=>{showAlert(item.description)}}> 
    <Text  style={styles.ttitle}> <MaterialIcons
      ...
      <Card.Content style={{paddingTop:20}} >
        <Text>Ticket .No: {item.collectiontaskTicketNumber}</Text>
       </Card.Content>
    ...
    return (
      <View style={styles.container}>
        ...
        <ScrollView style={styles.main1}>
          <View style={{ zIndex: -1 }}>
            <Text style={styles.head}>Completed Collection Tasks</Text>
