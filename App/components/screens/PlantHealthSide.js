...
import BaseUrl from "../api/BaseUrl";
  ...
  const PlantHealthSide = () => {
    ...
    const [TaskHealthPage, setTaskHealthPage] = useState([]);
    const navigation = useNavigation();
    const onTaskSide = (id) => {
     // console.warn("HI"+id)
      navigation.navigate("PlantHealthClick");
      AsyncStorage.setItem ("planthealthtaskId",JSON.stringify(id));
    };
    ...
    const getTaskHealthPage = () => {
      axios({
        method: "GET",
        url: `${BaseUrl}/planthealthtask/v1/getAllPlantHealthTasksByPagination/{pageNumber}/{pageSize}/{technicianId}?pageNumber=${pageNumber}&pageSize=${pageSize}&technicianId=${techId}`,
        headers: {
       ...
       // console.warn(result.data);
        ...
        setTaskHealthPage(result.data.content);
      });
    };
  ...
  const viewItem = ({item}) => {
    ...
    return (
      <Card style={styles.ccard} onPress={()=>{
        onTaskSide(item.planthealthtaskId)
      }}>
      <View style={{flexDirection:"row" ,backgroundColor:"skyblue"}}>
      <Text style={styles.ttitle}>{item.planthealthtaskName}</Text>
   <TouchableOpacity onPress = {()=>{showAlert(item.description)}}> 
    <Text  style={styles.ttitle}> <MaterialIcons
      ...
      <Card.Content style={{paddingTop:20}} >
        <Text>Ticket .No: {item.planthealthtaskTicketNumber}</Text>
       </Card.Content>
       <Card.Content style={{paddingTop:10}} > 
        ...
        <View >
        <Text >DeadLine: {moment(item.deadlineDate).format("LL")}</Text>
        </View>
      </Card.Content>
    </Card>
    )
    ...
    return (
      <View style={styles.container}>
        ...
        <ScrollView style={styles.main1}>
          <View style={{ zIndex: -1 }}>
            <Text style={styles.head}>Completed Plant Health Tasks</Text>
  ...
  export default PlantHealthSide;
