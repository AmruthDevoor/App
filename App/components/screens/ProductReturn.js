...
const ProductReturn = () => {
  ...
  const getProductReturn = () => {
    axios({
      method: "GET",
      url:  `${BaseUrl}/productreturn/v1/getReturnedProductsByPagination/{pageNumber}/{pageSize}/{technicianId}?pageNumber=${pageNumber}&pageSize=${pageSize}&technicianId=${techId}`,
      headers: {
...
const viewItem = ({item}) => {
  return (
     ...
     <Text style={styles.content}>Serial No: {item.serialNumber}</Text>
     </Card.Content>
     <Card.Content style={{ flexDirection: "row" }}>   
